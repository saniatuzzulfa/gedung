...
class pengeluaran extends BaseController
{
    ...
    {
        $mod = $this->mod;
        $cek = $this->db->table('tr_keluar')->select('no_bukti')->orderBy('no_bukti', 'DESC')->limit(1)->get()->getRow();
        if ($cek) {
            $kode = substr($cek->no_bukti, 6, 9);
            $kodene = sprintf("PL-%04s", ++$kode);
        } else {
            $kodene = "PL-0001";
        }
        ...
        $data = [
            'kode'        => $kodene,
            'kas'        => $this->mod->getWhere('kas', '*', ['tipe_kas' => 'pengeluaran'])->getResult()
        ];
    ...
    public function cetak_pengeluaran($tgl_awal = null, $tgl_akhir = null)
    {
        ...
        // $filename = date('y-m-d-H-i-s'). '-pengeluaran';

        //       // instantiate and use the dompdf class
        // $dompdf = new Dompdf();

        //       // load HTML content
        // $dompdf->loadHtml(view('admin/cetak_pengeluaran',$data));
